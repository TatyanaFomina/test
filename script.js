...
$bookList.on("click", "[data-id]", function (event) {
    ...
    $currentBook.fadeIn();
    $currentBook.find(".book-title").text(`${book.volumeInfo.title} | ${book.volumeInfo.authors ? book.volumeInfo.authors.join(", ") : "No author"} 
    ...
    $("<a>").attr("href", book.volumeInfo.previewLink).attr("target", "_blank").text("Read more...").addClass("read-link").appendTo($bookDescription);
});